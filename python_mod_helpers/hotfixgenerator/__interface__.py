...
def SelectionWindow(Func):
    ...
    def Get_Val(Type):
        ...
        # This will search the database for provided information
            ...
            # puts the daatabase results into a tkinter Text window so that the user can look at this information
            for Details in Info:
                if Details[0] not in DataBase_Results: DataBase_Results.append(Details[0])
            DataBase_Results.sort()
            ListBoxWindow(3) # Calls the function to display the 
    ...
    def Dis_Hotfix(ints):
        ...
        # but due to the way the code works is that it divides up the other answers and puts them in where they need to be 
            ...
            # This was all grabbed from the bl3hotfix file, as i wanted a way for users to see their hotfixes they are creating, and theire was no easy way of doing it
    ...
    if Func == "Mod Info":  # Creates a mod file of you to use
        ...
        # meaning that I can grab them for use later without having them assigned 
    ...
    # The reason I do it like this is so that I only create what I need for each different window, saving resources
    ...
    for i in range(1, Lab_Ent):
        ...
        for key, value in locals().items():
            ...
            if key == Entry_Var:
                if i <= dropBox: # this way I do not have to specify which one it is, I can just say how may times i want it to have a drop box
    ...
    # I have their values stored in temporary var so that I can but them into the button later once I have both of them
    for i in range(Butt):
    ...
    if HotFix_Options == 1:
        # functionality for the Label users see when they want to preview their hotfix
