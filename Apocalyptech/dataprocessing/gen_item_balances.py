...
parser = argparse.ArgumentParser(
        description='Generates item balance sheets, in both CSV and XLSX',
        )
parser.add_argument('-v', '--verbose',
        action='store_true',
        help='Display verbose output while generating',
        )
...
info_paragraphs = [
        ...
        various weapons and items in Borderlands 3.  This was compiled by
        investigating the raw Borderlands 3 data, using the JohnWickParse
...
# URL Block
url_paragraphs = [
        ("""For a separate sheet which gives counts of all possible gear, if you just want to look at some big numbers:""",
            'https://docs.google.com/spreadsheets/d/1S_yhhguQGXslpM5dL3CXa4SDf4W_Fu3fTjK_4Fhq5ag/edit?usp=sharing'),
        ("""shdw (not to be confused with shadowevil) has a sheet which goes into part behavior details and such, as well:""",
            'https://docs.google.com/spreadsheets/d/16b7bGPFKIrNg_cJm_WCMO6cKahexBs7BiJ6ja0RlD04/edit?usp=sharing'),
        ("""The code which generated this data can be found here:""",
            'https://github.com/BLCM/bl3mods/blob/master/Apocalyptech/dataprocessing/gen_item_balances.py'),
        ("""A version of this sheet with Wonderlands data can be found here:""",
            'https://docs.google.com/spreadsheets/d/1m8rFzQLta5iKF1pYXDj_AwcHFt-NpnAQ__Efmwhtw1s/edit'),
        ]
url_row = cur_row
...
changelog = [
        ('Feb 21, 2023', [
            "Convert generation script to write XLS directly instead of just CSV",
...
part_cache = {}
title_cache = {}
for (sheet_label, filename, filename_long, balances, man_col_name, type_col_name, partset_names) in [
        ('Weapons', 'gun_balances.csv', 'gun_balances_long.csv', gun_balances, 'Manufacturer/Name', 'Gun Type', None),
        ('Shields', 'shield_balances.csv', 'shield_balances_long.csv', shield_balances, 'Manufacturer/Name', None, [
            'BODY',
            'RARITY',
            'LEGENDARY AUG',
            'AUGMENT',
        ...
        ('Grenade Mods', 'grenade_balances.csv', 'grenade_balances_long.csv', grenade_balances, 'Manufacturer/Name', None, [
            'MANUFACTURER',
            'ELEMENT',
            'RARITY',
            'AUGMENT',
        ...
        ('Class Mods', 'com_balances.csv', 'com_balances_long.csv', com_balances, 'Character/Name', None, [
            'CHARACTER',
            'MODTYPE',
            'RARITY',
            'PRIMARY',
        ...
        ('Artifacts', 'artifact_balances.csv', 'artifact_balances_long.csv', artifact_balances, 'Type/Name', None, [
            'RARITY',
            'LEGENDARY ABILITY',
            'ABILITY',
            'PRIMARY',
    ...
    with open(filename, 'w') as odf:
        with open(filename_long, 'w') as odf_long:
            ...
            match sheet_label:
                ...
                case _:
                    col_width = 31
            ws.set_column(0, 0, col_width, reg_format)
            # Rarity
            ws.set_column(1, 1, 17, reg_format)
            # Balance
            ws.set_column(2+fixed_offset, 2+fixed_offset, 37, fixed_format)
            # Category
            ws.set_column(3+fixed_offset, 3+fixed_offset, 25, reg_format)
            # Min Parts
            ...
            # Part
            ws.set_column(7+fixed_offset, 7+fixed_offset, 45, fixed_format)
            # Dependencies
