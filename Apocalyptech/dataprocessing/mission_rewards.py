...
with open(out_file, 'w') as df:
    ...
    for (obj_name, obj) in mission_data:
        reward_pools_full = []
        if obj is None:
            # We couldn't serialize this mission (a known thing, it happens), so let's
            # try and glean it just from object refs instead
            found_pools = []
            for ref_name in bl3data.get_refs_from(obj_name):
                if 'ItemPool' in ref_name and '.' in ref_name:
                    found_pools.append(ref_name)
            if found_pools:
                if len(found_pools) == 1:
                    reward_pools_full = found_pools
                else:
                    print('WARNING: More than one pool found via refs for {}'.format(obj_name))
        else:
            for part in obj:
                if part['export_type'] == 'OakMissionRewardData':
                    if 'ItemPoolReward' in part:
                        reward_pools_full.append(part['ItemPoolReward']['asset_path_name'])

        # Now, if we found any pools, do stuff.
        if reward_pools_full:
            multiple_reward_tracker = []

            for reward_pool_full in reward_pools_full:
